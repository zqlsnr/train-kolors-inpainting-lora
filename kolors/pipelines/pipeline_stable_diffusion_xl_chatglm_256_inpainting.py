...
class StableDiffusionXLInpaintPipeline(
    ...
    def prepare_latents(
        ...
        # self.save_image(image_latents, '/aidata/lzq/image-outpaint/Kolors/inpainting/0814_test/4_vae.png')
    ...
    def save_image(self, image_latents, vae_image_path):
        if True:
            self.upcast_vae()
            image_latents = image_latents.to(next(iter(self.vae.post_quant_conv.parameters())).dtype)
        elif image_latents.dtype != self.vae.dtype:
            if torch.backends.mps.is_available():
                self.vae = self.vae.to(image_latents.dtype)
        ...
        torch.save(image_latents, 'image_latents_new.pt')
        # image_latents = torch.load('/aidata/lzq/image-outpaint/Kolors/inpainting/src/image_latents.pt')
